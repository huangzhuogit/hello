...
<table class="layui-table" lay-data="{cellMinWidth: 140, url:'<%=basePath%>hyzx/findAllByPage?is_delete=1', page:true, id:'demo',showLoad:true}" lay-filter="demo">
  <thead>
    <tr>
      <th lay-data="{type:'checkbox' ,fixed: 'left'}"></th>
      <th lay-data="{field:'hyzxTestImg', templet:imgFmt,fixed: true,align:'center'}">公司每个分类的图片</th>
	  <th lay-data="{field:'hyzxTestId'}">公司编号</th>
	  <th lay-data="{field:'hyzxTestName'}">公司每个分类的详细介绍</th>
      <th lay-data="{field:'hyzxTypeName'}">公司每个分类的名称</th>
         ...
         <div class="layui-upload-list">
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<input id="hyzxTestNameAdd" type="text" name="user.loginName"
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<input id="hyzxTypeNameAdd" type="text" name="user.loginPwd"
	 ...
	 <input id="uidupdate" type="hidden" name="id" >
         ...
         <div class="layui-upload-list">
			...
			<div class="layui-form-item">
				<label class="layui-form-label">公司每个分类的详细介绍:</label>
				<div class="layui-input-inline">
					<input id="hyzxTestNameUpdate" type="text" name="user.loginName"
						lay-verify="required" placeholder="请输入公司每个分类的详细介绍" autocomplete="off"
						class="layui-input">
				</div>
			</div>
			<div class="layui-form-item">
				<label class="layui-form-label">公司每个分类的名称:</label>
				<div class="layui-input-inline">
					<input id="hyzxTypeNameUpdate" type="text" name="user.loginPwd"
						lay-verify="required" placeholder="请输入公司每个分类的名称" autocomplete="off"
						class="layui-input">
				</div>
			</div>
...
//图片格式化
function imgFmt(d){
	return '<img style="width:120px;height:50px" src="hp_layui/img/'+d.hyzxTestImg+' " />'
} 
...
layui.use(['table','layer','form','laydate','upload'], function(){
 ...
 table.reload('demo',{
	  ...
	  ,url:'<%=basePath%>hyzx/findAllByPage'
	  ,where:{
		  'is_delete':0
	  }
 });
 ...
 table.reload('demo',{
  ...
  form.on('submit(addUI)', function(data){
	  ...
	  $.ajax({
	    	 type:'post',
	    	 url:'<%=basePath%>hyzx/addByUser',
	    	 data:{
	    		'hyzxTestName':$("#hyzxTestNameAdd").val(),
			    'hyzxTypeName':$("#hyzxTypeNameAdd").val(),
			    'hyzxTestImg':$("#addimgname").val(),
	    	 },
	    	 dataType:'json',
...
//监听提交
  form.on('submit(updateUserBut)', function(data){
    ...
    //异步提交    		
		     $.ajax({
		    	 type:'post',
		    	 url:'<%=basePath%>hyzx/goodUpdate',
		    	 data:{
		    		        'hyzxTestId':$("#uidupdate").val(),
						    'hyzxTestName':$("#hyzxTestNameUpdate").val(),
						    'hyzxTypeName':$("#hyzxTypeNameUpdate").val(),
						    'hyzxTestImg':$("#updateimgname").val(),
       ...
       /*  obj.del();
        layer.close(index);
    	  */
    	  ...
    	  $.ajax({
          	type:'post',
          	url:'<%=basePath%>hyzx/deleteOne',
          	data:{
          		'hyzxTestId':data.hyzxTestId
          	},
          	dataType:'json',
          	success:function(data){
          		var mes=data.mes;
        ...
        layer.confirm('真的还原吗', function(index){
        	...
        	layer.open({
      		  ...
      		  anim:1,
      		   success:function(){
      			   $("#uidupdate").val(data.hyzxTestId);
      			   $('#updateimg').attr('src','hp_layui/img/'+data.hyzxTestImg);
      			   $('#updateimgname').val(data.hyzxTestImg);
      			   $("#hyzxTestNameUpdate").val(data.hyzxTestName);
     			   $("#hyzxTypeNameUpdate").val(data.hyzxTypeName);
      		   },
