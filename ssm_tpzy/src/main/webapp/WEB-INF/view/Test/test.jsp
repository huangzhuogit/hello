...
<table class="layui-table" lay-data="{cellMinWidth: 140, url:'<%=basePath%>test/findAllByPage?is_delete=1', page:true, id:'demo',showLoad:true}" lay-filter="demo">
  <thead>
    <tr>
      <th lay-data="{type:'checkbox' ,fixed: 'left'}"></th>
	  <th lay-data="{field:'img', templet:imgFmt,fixed: true,align:'center'}">产品图片</th>
	  <th lay-data="{field:'id'}">产品编号</th>
	  <th lay-data="{field:'name'}">产品名</th>
      <th lay-data="{field:'mark'}">内容描述</th>
      <th lay-data="{field:'product'}">产品简介</th>
      <th lay-data="{field:'territory'}">适用领域</th>
      <th lay-data="{field:'tvideo'}">产品视频</th>
      ...
      <th lay-data="{field:'updateTime'}">修改时间</th>
   	  ...
   	  <th lay-data="{field:'type',templet:typefmt}">会员等级</th>
...
<div id="addUI" style="display: none;">
         ...
         <div class="layui-upload-list">
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<input id="nameAdd" type="text" name="user.loginName"
						lay-verify="required" placeholder="请输产品名" autocomplete="off"
						class="layui-input">
				</div>
			</div>
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<input id="markAdd" type="text" name="user.loginPwd"
						lay-verify="required|username" placeholder="请输入产品详情" autocomplete="off"
						class="layui-input">
				</div>
			</div>
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline" id="levelDiv">
					<select  name="type" lay-verify="" id="addFind">
		...
		<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<input id="productAdd" type="text" name="user.loginPwd"
						lay-verify="required" placeholder="请输入产品简介" autocomplete="off"
						class="layui-input">
				</div>
			</div>
			
			<div class="layui-form-item">
				<label class="layui-form-label">使用领域:</label>
				<div class="layui-input-inline">
					<input id="territoryAdd" type="text" name="user.loginPwd"
						lay-verify="required" placeholder="请输入使用领域" autocomplete="off"
						class="layui-input">
				</div>
			</div>
		 ...
		 <div style="width:350px;height:180px">
		<button type="button" class="layui-btn" id="addmoviebut"><i class="layui-icon"></i>上传视频</button>
	      <div class="layui-upload" style="text-align: center;">
			    <input id="addmoviename" type="hidden" name="home.Img">
	    </div>
	  </div> 
		 <div class="layui-form-item">
		...
		<div style="width:350px;height:180px">
         ...
         <div class="layui-upload-list">
			...
			<div class="layui-form-item">
				<label class="layui-form-label">产品名:</label>
				<div class="layui-input-inline">
					<input id="nameupdate" type="text" name="user.loginName"
			...
			<div class="layui-form-item">
				<label class="layui-form-label">内容描述:</label>
				<div class="layui-input-inline">
					<input id="markupdate" type="text" name="user.loginPwd"
						 placeholder="请输入产品详情" autocomplete="off"
						class="layui-input">
				</div>
			</div>
			...
			<div class="layui-form-item">
				<label class="layui-form-label">产品类型：</label>
				<div class="layui-input-inline" id="levelDivupdate">
					<select  name="type" lay-verify="" id="updateFind">
						<option value="">请选择</option>
					</select>
				</div>
			</div>
				...
				<label class="layui-form-label">产品简介:</label>
				<div class="layui-input-inline">
					<input id="productupdate" type="text" name="user.loginPwd"
						lay-verify="required" placeholder="请输入产品简介" autocomplete="off"
						class="layui-input">
				</div>
			</div>
			...
			<div class="layui-form-item">
				<label class="layui-form-label">使用领域:</label>
				<div class="layui-input-inline">
					<input id="territoryupdate" type="text" name="user.loginPwd"
						lay-verify="required" placeholder="请输入使用领域" autocomplete="off"
						class="layui-input">
				</div>
			</div>
			<div style="width:350px;height:180px">
		<button type="button" class="layui-btn" id="updatemoviebut"><i class="layui-icon"></i>上传视频</button>
	      <div class="layui-upload" style="text-align: center;">
			  <input id="updatemoviename" type="hidden" name="home.Img">
	    </div>
	  </div> 
...
layui.use(['table','layer','form','laydate','upload'], function(){
 ...
 table.reload('demo',{
	  ...
	  ,url:'<%=basePath%>test/findAllByPage'
	  ,where:{
		  'is_delete':0
	  }
 });
 ...
 table.reload('demo',{
   ...
   //下拉类型实现
    $(function(){
	  ...
	  $.ajax({
      	type:'post',
      	url:'<%=basePath%>type/showAlllevel',
      	dataType:'json',
      	success:function(data){
      		var uts=data.levlist;
      	  var cityhtml="";
      		for (var i = 0; i < uts.length; i++) {
      			/* 动态option拼接  <option value="">请选择</option> */
				cityhtml+='<option value="'+uts[i].tid+'">'+uts[i].tname+'</option>';
			}
      		//追加到select中
      		$("#addFind").append(cityhtml);	
      		$("#updateFind").append(cityhtml);
      		//表单重写渲染
  ...
  form.on('submit(addUI)', function(data){
	  debugger;
	      var videoVal=$("#addmoviename").val();
	  $.ajax({
	    	 type:'post',
	    	 url:'<%=basePath%>test/addByUser',
	    	 data:{
	    		'name':$("#nameAdd").val(),
			    'mark':$("#markAdd").val(),
			    'type.tid':$("#addFind").val(),
			    'img':$("#addimgname").val(),
			    'product':$("#productAdd").val(),
			    'territory':$("#territoryAdd").val(),
			    'tvideo':videoVal,
			    'is_delete':1,
	    	 },
	    	 dataType:'json',
...
//监听提交
  form.on('submit(updateUserBut)', function(data){
    ...
    //异步提交    		
		     $.ajax({
		    	 type:'post',
		    	 url:'<%=basePath%>test/goodUpdate',
		    	 data:{
		    		     'id':$("#uidupdate").val(), 		    	 
			    		 'img':$("#updateimgname").val(),
			    		 'type.tid':$("#updateFind").val(),
			    		 'name':$("#nameupdate").val(),
			    		 'mark':$("#markupdate").val(),
			    		 'product':$("#productupdate").val(),
						 'territory':$("#territoryupdate").val(),
						 'tvideo':$("#updatemoviename").val()
		    	 },
		    	 dataType:'json',
		    	 success:function(data){
		    		 var mes=data.mes;
       ...
       /*  obj.del();
        ...
        layer.confirm('真的还原吗', function(index){
        	...
        	layer.open({
      		  ...
      		  anim:1,
      		   success:function(){
      			   $("#uidupdate").val(data.id);
      			 $('#updateimg').attr('src','hp_layui/img/'+data.img);
     			   $("#levelDivupdate dd[lay-value="+data.type.tid+"]").click();
        		 $('#updateimgname').val(data.img);
     			   $("#nameupdate").val(data.name);
     			  $("#markupdate").val(data.mark);
     			 $("#productupdate").val(data.product);
     			$("#territoryupdate").val(data.territory);
                 form.render('select');
      		   },
    		   cancel: function(){ 
    		     //右上角关闭回调
  ...
  var uploadInst = upload.render({
     ...
     upload.render({
	    ...
	    elem: '#addmoviebut,#updatemoviebut'
	    ,url: '<%=basePath%>type/fileUpload'
	    ,accept: 'video' //视频
	    ,type:'post'
	    ,done: function(res){
	      console.log(res)
	      if(res.mes=="succ"){
		    	//上传成功
		    	$("#addmoviename").val(res.fileName);
		    	$("#updatemoviename").val(res.fileName);
		        layer.msg('上传成功');
		        console.log(res);
		      }else{
		    	  //如果上传失败
